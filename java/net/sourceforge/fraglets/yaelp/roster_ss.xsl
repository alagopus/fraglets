...
<xsl:template match="heading"><TR STYLE="background-color: #000080; color: white;"><xsl:apply-templates/></TR></xsl:template>
...
<xsl:template match="avatar"><TR style="background-color: #000080; color: white;"><xsl:apply-templates/></TR></xsl:template>
<xsl:template match="guild|zone"/>
<xsl:template match="name|culture|class|level|time"><TD><xsl:apply-templates/>&#xa0;</TD></xsl:template>
</xsl:stylesheet>